...
def calculate_heat_pump_boost(p_wh_use, T_in_C, boost_enabled=False, boosted_temp_c=None):
    ...
    Important:
    - For the Word 5/22/26 water formulation, E_in_WR comes from VTAS/EES/data.
    - For CCS in Version 8, this helper is also used internally to calculate automatic
      heat-pump boosting when the case waste-heat temperature is below the required
      CCS process temperature.
    """
...
def calculate_ccs_process_from_inputs(
    q_waste_available_kwhth_per_kwh_it,
    phi_use,
    t_waste_c,
    required_temperature_c,
    thermal_energy_kwhth_per_kgco2,
    direct_electricity_kwh_per_kgco2,
    e_it_kwh,
    asic=False,
):
    """
    Calculate the actual waste-heat-assisted CCS process outputs used by the
    Word CCS URE formulation.

    Dataset/process inputs:
        q_waste_available_kwhth_per_kwh_it:
            Recoverable waste heat available for the selected FCAT case,
            normalized by E_IT [kWh_th/kWh_IT].
        phi_use:
            Fraction of available waste heat sent to the CCS process [0-1].
        t_waste_c:
            Waste-heat temperature for the selected FCAT case [degC].
        required_temperature_c:
            Required CCS regeneration/process temperature [degC].
        thermal_energy_kwhth_per_kgco2:
            Actual CCS process thermal-energy requirement [kWh_th/kgCO2e].
        direct_electricity_kwh_per_kgco2:
            Actual CCS direct electricity requirement excluding heat-pump
            boosting [kWh/kgCO2e].
        e_it_kwh:
            IT energy basis [kWh].
    ...
    Internally calculated outputs:
        Q_waste_use
        heat-pump COP and boost electricity, when needed
        Q_heat_delivered
        m_CO2e_removed
        direct electricity
        total E_in_CCS
    """
    q_waste_available_kwhth_per_kwh_it = float(q_waste_available_kwhth_per_kwh_it)
    phi_use = float(phi_use)
    t_waste_c = float(t_waste_c)
    required_temperature_c = float(required_temperature_c)
    thermal_energy_kwhth_per_kgco2 = float(thermal_energy_kwhth_per_kgco2)
    direct_electricity_kwh_per_kgco2 = float(direct_electricity_kwh_per_kgco2)
    e_it_kwh = float(e_it_kwh)

    if q_waste_available_kwhth_per_kwh_it < 0:
        raise ValueError("CCS available waste heat must be non-negative.")
    if not 0.0 <= phi_use <= 1.0:
        raise ValueError("CCS used waste heat fraction must be between 0 and 1.")
    if thermal_energy_kwhth_per_kgco2 <= 0:
        raise ValueError("CCS thermal-energy requirement must be greater than zero.")
    if direct_electricity_kwh_per_kgco2 < 0:
        raise ValueError("CCS direct electricity requirement must be non-negative.")
    if e_it_kwh <= 0:
        raise ValueError("E_IT must be greater than zero.")

    effective_t_waste_c = t_waste_c + (5.0 if asic else 0.0)
    q_waste_available_kwhth = q_waste_available_kwhth_per_kwh_it * e_it_kwh
    q_waste_use_kwhth = phi_use * q_waste_available_kwhth

    ccs_boost_required = required_temperature_c > effective_t_waste_c

    if ccs_boost_required:
        ccs_boost_outputs = calculate_heat_pump_boost(
            p_wh_use=q_waste_use_kwhth,
            T_in_C=effective_t_waste_c,
            boost_enabled=True,
            boosted_temp_c=required_temperature_c,
        )
    else:
        ccs_boost_outputs = calculate_heat_pump_boost(
            p_wh_use=q_waste_use_kwhth,
    ...
    q_heat_delivered_kwhth = ccs_boost_outputs["Q_heat_to_offtaker"]
    e_boost_ccs_kwh = ccs_boost_outputs["E_boost"]

    m_co2e_removed_kg = q_heat_delivered_kwhth / thermal_energy_kwhth_per_kgco2
    e_direct_ccs_kwh = m_co2e_removed_kg * direct_electricity_kwh_per_kgco2
    e_in_ccs_total_kwh = e_direct_ccs_kwh + e_boost_ccs_kwh

    return {
        "q_waste_available_kwhth_per_kwh_it": q_waste_available_kwhth_per_kwh_it,
        "q_waste_available_kwhth": q_waste_available_kwhth,
        "phi_use": phi_use,
        "q_waste_use_kwhth": q_waste_use_kwhth,
        "t_waste_c": t_waste_c,
        "effective_t_waste_c": effective_t_waste_c,
        "required_temperature_c": required_temperature_c,
        "boost_required": ccs_boost_required,
        "boost_enabled": ccs_boost_outputs["boost_enabled"],
        "cop_hp": ccs_boost_outputs["COP_HP"],
        "e_boost_ccs_kwh": e_boost_ccs_kwh,
        "q_heat_delivered_kwhth": q_heat_delivered_kwhth,
        "offtaker_inlet_temperature_c": ccs_boost_outputs["T_offtaker_in_C"],
        "thermal_energy_kwhth_per_kgco2": thermal_energy_kwhth_per_kgco2,
        "direct_electricity_kwh_per_kgco2": direct_electricity_kwh_per_kgco2,
        "m_co2e_removed_kg": m_co2e_removed_kg,
        "m_co2e_removed_kg_per_kwh_it": m_co2e_removed_kg / e_it_kwh,
        "e_direct_ccs_kwh": e_direct_ccs_kwh,
        "e_direct_ccs_kwh_per_kwh_it": e_direct_ccs_kwh / e_it_kwh,
        "e_in_ccs_kwh": e_in_ccs_total_kwh,
        "e_in_ccs_kwh_per_kwh_it": e_in_ccs_total_kwh / e_it_kwh,
        "hp_cop_model": ccs_boost_outputs["HP_COP_model"],
    }
...
def calculate_ure_ccs_word(
    ...
    """
    Carbon capture and storage URE using the final CCS formulation.

    Actual net benefit:
        NB_CCS = Delta CF = m_CO2e - E_in * CEF

    Baseline conversion factor:
        beta_CCS = (m_CO2e / E_in)_baseline - CEF

    If E_in = 0:
        URE_CCS = m_CO2e / (beta_CCS * E_IT)

    If E_in > 0:
        URE_CCS = [((m_CO2e/E_in) - CEF) / beta_CCS - 1] * (E_in/E_IT)

    The equivalent general form is:
        URE_CCS = NB_CCS / (beta_CCS * E_IT) - E_in / E_IT

    Units:
        m_CO2e_removed: kg CO2e
        E_in_CCS: kWh
        CEF: kg CO2e/kWh
        baseline_ccs_kgco2e_per_kwh: kg CO2e removed per kWh
        E_IT: kWh
    """
    ...
    if beta_ccs <= 0:
        return {
            ...
            "formula_case": "E_in = 0" if e_in_ccs_kwh == 0 else "E_in > 0",
            "status": "not_applicable",
            "message": (
                "CCS URE is not applicable because beta_CCS <= 0. "
                "The baseline carbon-removal effectiveness must be greater than CEF."
            ),
            "minimum_required_CCS_baseline_kgCO2e_per_kWh": cef_kgco2e_per_kwh,
        }

    if e_in_ccs_kwh == 0:
        ure_ccs = m_co2e_removed_kg / (beta_ccs * e_it_kwh)
        formula_case = "E_in = 0"
    else:
        ure_ccs = (
            ((actual_ccs_kg_per_kwh - cef_kgco2e_per_kwh) / beta_ccs) - 1.0
        ) * (e_in_ccs_kwh / e_it_kwh)
        formula_case = "E_in > 0"

    e_nb_ccs = ure_ccs * e_it_kwh
...
def load_pue_table(file_path):
    ...
    optional_cols = [
        ...
        # Version 8 CCS process inputs. Derived CCS values are calculated in Python.
        "CCS_Q_waste_available_kWhth_per_kWh_IT",
        "CCS_T_waste_C",
        "CCS_required_temperature_C",
        "CCS_actual_thermal_energy_kWhth_per_kgCO2",
        "CCS_actual_direct_electricity_kWh_per_kgCO2",
...
def calculate_outputs(
    ...
    if application == "ORC":
        ...
        if ccs_override_params:
            q_waste_available_kwhth_per_kwh_it = float(
                ccs_override_params["q_waste_available_kwhth_per_kwh_it"]
            )
            t_waste_c = float(ccs_override_params["t_waste_c"])
            required_temperature_c = float(ccs_override_params["required_temperature_c"])
            thermal_energy_kwhth_per_kgco2 = float(
                ccs_override_params["thermal_energy_kwhth_per_kgco2"]
            )
            direct_electricity_kwh_per_kgco2 = float(
                ccs_override_params["direct_electricity_kwh_per_kgco2"]
            )
            cef_kgco2e_per_kwh = float(ccs_override_params["cef_kgco2e_per_kwh"])
            baseline_ccs_kgco2e_per_kwh = float(
                ccs_override_params["baseline_ccs_kgco2e_per_kwh"]
            )
            ccs_data_source = "Manual override of CCS process inputs"
        else:
            q_waste_available_kwhth_per_kwh_it = value_from_row_float(
                row, "CCS_Q_waste_available_kWhth_per_kWh_IT"
            )
            t_waste_c = value_from_row_float(row, "CCS_T_waste_C")
            required_temperature_c = value_from_row_float(
                row, "CCS_required_temperature_C"
            )
            thermal_energy_kwhth_per_kgco2 = value_from_row_float(
                row, "CCS_actual_thermal_energy_kWhth_per_kgCO2"
            )
            direct_electricity_kwh_per_kgco2 = value_from_row_float(
                row, "CCS_actual_direct_electricity_kWh_per_kgCO2"
            )
            cef_kgco2e_per_kwh = value_from_row_float(
                row, "CCS_CEF_kgCO2e_per_kWh"
            )
            baseline_ccs_kgco2e_per_kwh = value_from_row_float(
                row, "CCS_baseline_kgCO2e_per_kWh"
            )
            ccs_data_source = "CCS process and baseline inputs from input file"

        required_values = {
            "available waste heat per kWh IT": q_waste_available_kwhth_per_kwh_it,
            "waste heat temperature": t_waste_c,
            "required CCS temperature": required_temperature_c,
            "actual CCS thermal energy per kg CO2": thermal_energy_kwhth_per_kgco2,
            "actual CCS direct electricity per kg CO2": direct_electricity_kwh_per_kgco2,
        ...
        if missing_values:
            raise ValueError(
                "Carbon capture Version 8 requires: "
                + ", ".join(missing_values)
                + ". Fill these input columns or use the CCS manual override."
            )

        ccs_process = calculate_ccs_process_from_inputs(
            q_waste_available_kwhth_per_kwh_it=q_waste_available_kwhth_per_kwh_it,
            phi_use=phi_use,
            t_waste_c=t_waste_c,
            required_temperature_c=required_temperature_c,
        ...
        # For CCS only, replace the generic case assumptions with the CCS dataset inputs.
        q_avail = q_waste_available_kwhth_per_kwh_it
        p_wh_avail = ccs_process["q_waste_available_kwhth"]
        p_wh_use = ccs_process["q_waste_use_kwhth"]
        effective_temp = ccs_process["effective_t_waste_c"]
        e_boost = ccs_process["e_boost_ccs_kwh"]
        p_total_to_offtaker = ccs_process["q_heat_delivered_kwhth"]
        t_offtaker_in = ccs_process["offtaker_inlet_temperature_c"]

        boost_outputs = {
            "boost_enabled": ccs_process["boost_enabled"],
            "T_source_C": ccs_process["effective_t_waste_c"],
            "T_offtaker_in_C": ccs_process["offtaker_inlet_temperature_c"],
            "COP_HP": ccs_process["cop_hp"],
            "E_boost": ccs_process["e_boost_ccs_kwh"],
            "Q_heat_to_offtaker": ccs_process["q_heat_delivered_kwhth"],
            "HP_COP_model": ccs_process["hp_cop_model"],
        }

        # ERF and ERE use reused data-center waste heat before heat-pump electricity addition.
        erf = p_wh_use / p_dc if p_dc != 0 else np.nan
        ere = (p_dc - p_wh_use) / p_it

        ccs_details = calculate_ure_ccs_word(
            m_co2e_removed_kg=ccs_process["m_co2e_removed_kg"],
            e_in_ccs_kwh=ccs_process["e_in_ccs_kwh"],
            cef_kgco2e_per_kwh=cef_kgco2e_per_kwh,
            baseline_ccs_kgco2e_per_kwh=baseline_ccs_kgco2e_per_kwh,
            e_it_kwh=e_it,
        )

        ccs_details.update(ccs_process)
        ...
        ure_basis = (
            "CCS final formulation: "
            "NB_CCS = m_CO2e - E_in_CCS*CEF; "
            "beta_CCS = (m_CO2e/E_in)_baseline - CEF; "
            "URE_CCS = NB_CCS/(beta_CCS*E_IT) - E_in_CCS/E_IT. "
            "m_CO2e and E_in_CCS are calculated internally from available waste heat, "
            "required CCS temperature, process thermal energy, direct electricity, and automatic boosting."
        )
    ...
    outputs = {
        ...
        "boosted_temp_c": (
            t_offtaker_in
            if application == "Carbon capture and storage" and boost_outputs["boost_enabled"]
            else (boosted_temp_c if boost_outputs["boost_enabled"] else None)
        ),
...
if application == "Carbon capture and storage":
    ...
    st.caption(
        "For CCS, the base waste-heat temperature is read from CCS_T_waste_C in the input file. "
        "The ASIC option adds 5°C to that CCS dataset temperature."
    )
else:
...
# -------------------------------------------------------------------
if application == "Carbon capture and storage":
    st.subheader("CCS Temperature Boosting")
    boost_enabled = False
    boosted_temp_c = None
    st.info(
        "For CCS, heat-pump boosting is calculated automatically inside the Python code. "
        "Boosting is activated only when CCS_required_temperature_C is greater than the "
        "effective CCS waste-heat temperature read from the input file."
    )
else:
...
if application == "Carbon capture and storage":
    ccs_case_input_preview_df = pd.DataFrame([{
        "CCS available waste heat from file (kWhth/kWh_IT)": value_from_row_float(
            row, "CCS_Q_waste_available_kWhth_per_kWh_IT"
        ),
        "CCS waste heat temperature from file (°C)": value_from_row_float(
            row, "CCS_T_waste_C"
        ),
    }])
    st.dataframe(ccs_case_input_preview_df, use_container_width=True)
    st.caption(
        "For CCS only, the Python calculation uses these CCS dataset values instead of "
        "the hardcoded Q_avail factor and the general case temperature shown above."
    )
...
if application == "Water reclamation":
    ...
    st.subheader("Carbon Capture and Storage Process Inputs")

    file_q_waste = value_from_row_float(
        row, "CCS_Q_waste_available_kWhth_per_kWh_IT"
    )
    file_t_waste = value_from_row_float(row, "CCS_T_waste_C")
    file_t_required = value_from_row_float(row, "CCS_required_temperature_C")
    file_thermal_energy = value_from_row_float(
        row, "CCS_actual_thermal_energy_kWhth_per_kgCO2"
    )
    file_direct_electricity = value_from_row_float(
        row, "CCS_actual_direct_electricity_kWh_per_kgCO2"
    )
    ...
    ccs_preview_df = pd.DataFrame([{
        "Available waste heat, kWhth/kWh_IT": file_q_waste,
        "Waste heat temperature, °C": file_t_waste,
        "Required CCS temperature, °C": file_t_required,
        "Actual CCS thermal energy, kWhth/kgCO2": file_thermal_energy,
        "Actual CCS direct electricity, kWh/kgCO2": file_direct_electricity,
    ...
    st.caption(
        "Version 8 reads only the CCS process, location, and baseline inputs from the dataset. "
        "The Python code calculates usable heat, automatic temperature boosting, CO2 removed, "
        "direct electricity, total E_in_CCS, NB_CCS, beta_CCS, and URE_CCS."
    )

    ccs_manual = st.checkbox(
        "Override/add CCS process inputs manually",
        value=False,
        help="Use this while the CCS input columns are being developed.",
    )

    if ccs_manual:
        c1, c2, c3 = st.columns(3)
        with c1:
            ccs_q_waste_manual = st.number_input(
                "Available waste heat (kWhth/kWh_IT)",
                min_value=0.0,
                value=safe_default(file_q_waste, 0.0),
                step=0.01,
                format="%.6f",
            )
            ccs_t_waste_manual = st.number_input(
                "CCS waste heat temperature (°C)",
                value=safe_default(file_t_waste, default_temp),
                step=1.0,
                format="%.2f",
            )
        with c2:
            ccs_t_required_manual = st.number_input(
                "Required CCS process temperature (°C)",
                value=safe_default(file_t_required, 100.0),
                step=1.0,
                format="%.2f",
            )
            ccs_thermal_energy_manual = st.number_input(
                "Actual CCS thermal energy (kWhth/kgCO2)",
                min_value=0.000001,
                value=max(safe_default(file_thermal_energy, 0.001), 0.000001),
                step=0.01,
                format="%.6f",
            )
        with c3:
            ccs_direct_electricity_manual = st.number_input(
                "Actual CCS direct electricity (kWh/kgCO2)",
                min_value=0.0,
                value=safe_default(file_direct_electricity, 0.0),
                step=0.01,
                format="%.6f",
            )
            ...
            ccs_baseline_manual = st.number_input(
                ...
                min_value=0.000001,
                value=max(safe_default(file_baseline, 0.001), 0.000001),
                step=0.001,
                format="%.6f",
            )

        ccs_override_params = {
            "q_waste_available_kwhth_per_kwh_it": ccs_q_waste_manual,
            "t_waste_c": ccs_t_waste_manual,
            "required_temperature_c": ccs_t_required_manual,
            "thermal_energy_kwhth_per_kgco2": ccs_thermal_energy_manual,
            "direct_electricity_kwh_per_kgco2": ccs_direct_electricity_manual,
...
selected_inputs_df = pd.DataFrame([{
    ...
    "User-entered waste heat temperature (°C)": (
        None if application == "Carbon capture and storage" else temp
    ),
    "ASIC checked": asic,
...
if application == "ORC":
    ...
    results_row = {
        **common_results,
        "CCS available waste heat, kWhth/kWh_IT": outputs.get("CCS_q_waste_available_kwhth_per_kwh_it"),
        "CCS used waste heat before boost, kWhth": outputs.get("CCS_q_waste_use_kwhth"),
        "CCS waste heat temperature, °C": outputs.get("CCS_effective_t_waste_c"),
        "Required CCS temperature, °C": outputs.get("CCS_required_temperature_c"),
        "CCS boost required": outputs.get("CCS_boost_required"),
        "CCS heat pump COP": outputs.get("CCS_cop_hp"),
        "CCS heat delivered, kWhth": outputs.get("CCS_q_heat_delivered_kwhth"),
        "CCS thermal energy, kWhth/kgCO2": outputs.get("CCS_thermal_energy_kwhth_per_kgco2"),
        "CCS direct electricity intensity, kWh/kgCO2": outputs.get("CCS_direct_electricity_kwh_per_kgco2"),
        "m_CO2e removed, kg": outputs.get("CCS_m_co2e_removed_kg"),
        "Direct CCS electricity, kWh": outputs.get("CCS_e_direct_ccs_kwh"),
        "CCS boost electricity, kWh": outputs.get("CCS_e_boost_ccs_kwh"),
        "E_in_CCS total, kWh": outputs.get("CCS_e_in_ccs_kwh"),
        ...
        "CCS formula case": outputs.get("CCS_formula_case"),
        "CCS status": outputs.get("CCS_status"),
    }
...
if application == "Water reclamation":
    ...
    ccs_status = outputs.get("CCS_status")
    if ccs_status == "not_applicable":
        st.warning(
            "⚠️ CCS URE is not applicable in this region because β_CCS ≤ 0. "
            "The baseline carbon-removal effectiveness must be greater than the local CEF."
        )
...
ure_structure_df = pd.DataFrame([
    ...
    },
    {
        ...
        "Status in current tool": "Active in Version 8",
        "Net benefit, NB": "m_CO2e_removed - E_in_CCS*CEF",
        "Baseline beta_NB": "CCS_baseline_kgCO2e_per_kWh - CEF",
        "URE formulation": "NB_CCS/(beta_CCS*E_IT) - E_in_CCS/E_IT",
        "Required model/data": (
            "Q_waste available, T_waste, required CCS temperature, actual thermal energy, "
            "actual direct electricity, CEF, and fixed baseline kgCO2e/kWh"
        ),
    },
])
...
st.info(
    ...
    "Version 8 calculates the actual CCS process outputs and final CCS URE internally from the CCS dataset inputs."
)
...
if application == "ORC":
    ...
    notes["CCS Q_waste available, kWhth/kWh_IT"] = (
        f"{outputs.get('CCS_q_waste_available_kwhth_per_kwh_it'):.6f}"
    )
    notes["CCS Q_waste used before boost, kWhth"] = (
        f"{outputs.get('CCS_q_waste_use_kwhth'):.6f}"
    )
    notes["CCS waste heat temperature, C"] = (
        f"{outputs.get('CCS_effective_t_waste_c'):.2f}"
    )
    notes["CCS required temperature, C"] = (
        f"{outputs.get('CCS_required_temperature_c'):.2f}"
    )
    notes["CCS boost required"] = outputs.get("CCS_boost_required")
    notes["CCS heat pump COP"] = (
        f"{outputs.get('CCS_cop_hp'):.6f}"
        if valid_number(outputs.get("CCS_cop_hp"))
        else "N/A"
    )
    notes["CCS heat delivered, kWhth"] = (
        f"{outputs.get('CCS_q_heat_delivered_kwhth'):.6f}"
    )
    notes["CCS thermal energy, kWhth/kgCO2"] = (
        f"{outputs.get('CCS_thermal_energy_kwhth_per_kgco2'):.6f}"
    )
    notes["CCS direct electricity intensity, kWh/kgCO2"] = (
        f"{outputs.get('CCS_direct_electricity_kwh_per_kgco2'):.6f}"
    )
    notes["m_CO2e removed, kg"] = f"{outputs.get('CCS_m_co2e_removed_kg'):.6f}"
    notes["Direct CCS electricity, kWh"] = f"{outputs.get('CCS_e_direct_ccs_kwh'):.6f}"
    notes["CCS boost electricity, kWh"] = f"{outputs.get('CCS_e_boost_ccs_kwh'):.6f}"
    notes["E_in_CCS total, kWh"] = f"{outputs.get('CCS_e_in_ccs_kwh'):.6f}"
    ...
    notes["CCS formula case"] = outputs.get("CCS_formula_case")
    notes["CCS status"] = outputs.get("CCS_status")
    notes["Important CCS implementation"] = (
        "The dataset provides the CCS process and baseline inputs. "
        "Python calculates temperature boosting, m_CO2e, total E_in_CCS, NB_CCS, beta_CCS, and URE_CCS."
    )
...
st.caption(
    ...
    "Water parameters should therefore be provided per kWh_IT unless using manual values consistently. "
    "HPD baseline for water is left as an operating-condition-dependent input for later literature/EES work. "
    "For CCS, Version 8 reads process intensities and the fixed baseline from the dataset, then calculates all derived CCS outputs internally."
)