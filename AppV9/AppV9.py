...
# Page setup
# -------------------------------------------------------------------
st.set_page_config(page_title="PURE — Platform for Understanding and Reusing Energy (v9)", layout="wide")
...
# --- v9: grid metrics vintage -------------------------------------------------
# The 2023 rebuild reports water intensity on three bases. They differ in what
# counts as water use, not in how URE is calculated:
#
#   consumption, no hydro   water evaporated at power plants, reservoir
#                           evaporation excluded. This is the headline basis.
#   consumption, with hydro adds reservoir evaporation from hydropower, at the
#                           Peer et al. regional values. Upper end of the range.
#   withdrawal              water taken from the source, most of which a
#                           once-through plant returns. Much larger numbers.
#
# Only the column changes. beta_WR is still EWIF and the formula is still
# URE_WR = (V_w / EWIF - E_in_WR) / E_IT.
EWIF_BASIS_COLUMNS = {
    "Consumption, no hydropower (default)": "WR_EWIF_L_per_kWh",
    "Consumption, hydropower included": "WR_EWIF_hydro_L_per_kWh",
    "Withdrawal": "WR_EWIF_withdrawal_L_per_kWh",
    "v8 keying (reference-city county)": "WR_EWIF_refcounty_L_per_kWh",
}
DEFAULT_EWIF_BASIS = "Consumption, no hydropower (default)"


def selected_ewif_column(df=None):
    """Column the app should read for EWIF, falling back to the v8 column."""
    label = st.session_state.get("ewif_basis", DEFAULT_EWIF_BASIS)
    col = EWIF_BASIS_COLUMNS.get(label, "WR_EWIF_L_per_kWh")
    if df is not None and col not in df.columns:
        return "WR_EWIF_L_per_kWh"
    return col


def ewif_from_row(row):
    """EWIF for this row on the selected basis, with a fallback."""
    col = st.session_state.get("_ewif_col", "WR_EWIF_L_per_kWh")
    value = value_from_row_float(row, col)
    if (value is None or (isinstance(value, float) and np.isnan(value))) \
            and col != "WR_EWIF_L_per_kWh":
        value = value_from_row_float(row, "WR_EWIF_L_per_kWh")
    return value
...
def load_pue_table(file_path):
    ...
    optional_cols = [
        # v9 grid metrics: alternative water bases and the 2019 values.
        "WR_EWIF_hydro_L_per_kWh",
        "WR_SWI_hydro_L_per_kWh",
        "WR_EWIF_withdrawal_L_per_kWh",
        "WR_SWI_withdrawal_L_per_kWh",
        "WR_EWIF_refcounty_L_per_kWh",
        "CCS_CEF_2019_kgCO2e_per_kWh",
        "WR_EWIF_2019_L_per_kWh",
        "WR_SWI_2019_L_per_kWh",
...
def calculate_outputs(
    ...
    if application == "Power generation using an organic Rankine cycle (ORC)":
        ...
        if wr_override_params:
            ...
            ewif_l_per_kwh = ewif_from_row(row)
...
# --- v9: which EWIF basis to use -----------------------------------------
available = [lab for lab, col in EWIF_BASIS_COLUMNS.items()
             if col in df.columns] or [DEFAULT_EWIF_BASIS]
default_index = (available.index(DEFAULT_EWIF_BASIS)
                 if DEFAULT_EWIF_BASIS in available else 0)
st.sidebar.markdown("### Grid metrics")
st.sidebar.selectbox(
    ...
    key="ewif_basis",
    help=(
        "Which water intensity the water reclamation URE uses as beta_WR. "
        "Consumption is water evaporated and not returned; withdrawal is "
        "water taken from the source, most of which is returned. "
        "Hydropower reservoir evaporation is reported separately because "
        "published values for it range from negative in New England to "
        "126 L/kWh in the Southwest."
    ),
)
st.session_state["_ewif_col"] = selected_ewif_column(df)
if "Grid_metrics_vintage" in df.columns:
    st.sidebar.caption("Grid metrics: 2023 reference year")
else:
    st.sidebar.caption("Grid metrics: v8 input file (2019 reference year)")
